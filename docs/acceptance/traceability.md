...
| FR-2 steady-state warmup | `cargo test warmup::` (golden synthetic sequences), `bench.rs::cold_ramp_run_is_trimmed_to_steady_state` | Playbook step 4; `l2` kernel where `warmup=25` is > 10% low | pending |
| FR-3 arch-aware L2 flush | `cargo test pipeline::flush_buffer_bytes` (sized from L2, not a constant) | Playbook step 5 (flush A/B ratios) | pending |
| FR-4 clock control | `cargo test bench::` (`a_hard_lock_error_degrades_to_an_unlocked_run`, throttled-sample drop), `replay.rs` clock-lock flows | Playbook step 9 (power-cap throttle) + step 8 locked; golden box | pending |
| FR-5 small-kernel batched | `cargo test graph::` (`should_capture` / `resolve`), `bench.rs::auto_graph_*` | Playbook step 10 write-up delta; `l2` 5 µs oracle (NFR-2) | pending |
| FR-6 `launch_overhead_us` | `cargo test oracles::` (O4 analytic), `bench.rs` graph vs eager | `l2` vs `nsys` API-call→kernel-start gap on ≥ 3 archs | pending |
...
| FR-8 occupancy | `cargo test occupancy::` vs `tests/occupancy/reference.csv`, `tests/l0_unit/test_occupancy.py` | Playbook step 7 (`achieved` vs `ncu` ±0.05, golden box) | pending |
...
| FR-13 `sweep` + spec + `Grid` + `--resume` + autotune cache | `cargo test spec::` + golden `appendix_d`, `cargo test autotune::`, `tests/l1_contract/test_sweep.py`, `tests/l6_e2e/test_autotune_cache.py` (skips off-GPU) | Playbook step 11 (kill + `--resume`); `l6` 30-cell sweep to a valid Parquet | pending |
...
| FR-16 `caliper submit` + results repo + CI | `cargo test submit::` (manifest, `validate_bundle`, all 4 rejection classes, tamper-resistant), `tests/l0_unit/test_submit.py`, `results-repo/.github/workflows/validate.yml` | Playbook step 13 (`submit --dry-run` on real rows → `validate bundle/`) | **CI** |
...
| FR-19 `caliper fingerprint` | `tests/l1_contract/test_cli.py::test_fingerprint_from_a_recording` (matches the `machine` block) | Playbook step 2 | **CI** |
